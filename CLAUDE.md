...
- `src/index.js`: the entire site. A `handler` routes `/robots.txt`, `/llms.txt`,
  `/llms-full.txt`, `/sitemap.xml`, and otherwise serves the `HTML` template-literal
  page; the handler is wrapped for bot tracking and exported as the Worker `fetch`.
...
The site exposes the files LLMs and AI crawlers look for. All are generated by the
Worker from constants in `src/index.js` (not static files on disk):

- `/llms.txt` (llmstxt.org standard): a concise, machine-readable map. H1 + summary
  blockquote + link sections (Documentation, Source/install, Commands, Optional). This
  is the primary entry point for an LLM indexing the site.
- `/llms-full.txt`: the full landing-page content as plain markdown, for deep ingestion.
- `/sitemap.xml`: standard sitemap; referenced from `robots.txt`.
- `/robots.txt`: `Allow: /` for all agents (welcomes AI crawlers) plus the `Sitemap:` line.
- JSON-LD `SoftwareApplication` structured data + a `<link rel="alternate">` hint to
  `/llms.txt` are embedded in the page `<head>`.

When the landing-page copy or command set changes, keep `LLMS_TXT` / `LLMS_FULL_TXT`
and the JSON-LD in sync. All of this content must also follow the no-em-dash rule below.
